...
class Xs_Maps_Widget extends Widget_Base {
    ...
    public function get_title() {
        return esc_html__( 'charitious Map', 'charitious' );
    }
	...
	protected function register_controls() {
		...
		$this->start_controls_section(
			...
			array(
				'label' => esc_html__( 'Map Settings', 'charitious' ),
			)
		);

		$default_address = esc_html__( 'London Eye, London, United Kingdom', 'charitious' );

		$this->add_control(
			'map_center',
			array(
				'label'       => esc_html__( 'Map Center', 'charitious' ),
		...
		$this->add_control(
			'zoom',
			array(
				'label'      => esc_html__( 'Initial Zoom', 'charitious' ),
		...
		$this->add_control(
			'scrollwheel',
			array(
				'label'   => esc_html__( 'Scrollwheel Zoom', 'charitious' ),
				'type'    => Controls_Manager::SELECT,
				'default' => 'false',
				'options' => array(
					'true'  => esc_html__( 'Enabled', 'charitious' ),
					'false' => esc_html__( 'Disabled', 'charitious' ),
				),
			)
		);
		...
		$this->add_control(
			'zoom_controls',
			array(
				'label'   => esc_html__( 'Zoom Controls', 'charitious' ),
				'type'    => Controls_Manager::SELECT,
				'default' => 'true',
				'options' => array(
					'true'  => esc_html__( 'Show', 'charitious' ),
					'false' => esc_html__( 'Hide', 'charitious' ),
				),
			)
		);
		...
		$this->add_control(
			'street_view',
			array(
				'label'   => esc_html__( 'Street View Controls', 'charitious' ),
				'type'    => Controls_Manager::SELECT,
				'default' => 'true',
				'options' => array(
					'true'  => esc_html__( 'Show', 'charitious' ),
		...
		$this->add_control(
			'map_type',
			array(
				'label'   => esc_html__( 'Map Type Controls (Map/Satellite)', 'charitious' ),
				'type'    => Controls_Manager::SELECT,
				'default' => 'true',
				'options' => array(
					'true'  => esc_html__( 'Show', 'charitious' ),
		...
		$this->add_control(
			'drggable',
			array(
				'label'   => esc_html__( 'Is Map Draggable?', 'charitious' ),
				'type'    => Controls_Manager::SELECT,
				'default' => 'true',
				'options' => array(
					'true'  => esc_html__( 'Yes', 'charitious' ),
					'false' => esc_html__( 'No', 'charitious' ),
				),
			)
		);
		...
		$this->start_controls_section(
			'section_map_style',
			array(
				'label' => esc_html__( 'Map Style', 'charitious' ),
			)
		);
		...
		$this->add_control(
			'map_height',
			array(
				'label'       => esc_html__( 'Map Height', 'charitious' ),
				...
				'selectors' => array(
					'{{WRAPPER}} #xs-map' => 'height: {{VALUE}}px',
				),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'label_block' => true,
				'description' => esc_html__( 'You can add own map styles within your theme. Add file with styles array in .json format into charitious/google-map-styles/ folder in your theme. File must be minified', 'charitious' )
			)
		);
		...
		$this->start_controls_section(
			'section_map_pins',
			array(
				'label' => esc_html__( 'Pins', 'charitious' ),
			)
		);
		...
		$repeater->add_control(
			'pin_address',
			array(
				'label'       => esc_html__( 'Pin Address', 'charitious' ),
				'type'        => Controls_Manager::TEXT,
				'default'     => $default_address,
				'label_block' => true,
			)
		...
		$repeater->add_control(
			'pin_desc',
			array(
				'label'   => esc_html__( 'Pin Description', 'charitious' ),
		...
		$repeater->add_control(
			'pin_image',
			array(
				'label'   => esc_html__( 'Pin Icon', 'charitious' ),
		...
		$repeater->add_control(
			'pin_state',
			array(
				'label'   => esc_html__( 'Initial State', 'charitious' ),
				'type'    => Controls_Manager::SELECT,
				'default' => 'visible',
				'options' => array(
					'visible' => esc_html__( 'Vsible', 'charitious' ),
					'hidden'  => esc_html__( 'Hidden', 'charitious' ),
				),
			)
		);
	...
	public function xs_get_map_style() {
		...
		$style = array(
			...
			'default' => esc_html__( 'Default', 'charitious' ),
	...
	public function get_location_coord( $location ) {
        ...
        $api_key  = charitious_option('map_api', charitious_defaults('map_api'));
	...
	protected function render() {
        ...
        printf(
            '<div class="xs-maps-wraper"><div id="xs-map" class="xs-box-shadow-2" %1$s %2$s></div></div>',
